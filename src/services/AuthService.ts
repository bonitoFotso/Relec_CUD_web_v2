...
class AuthService {
  ...
  private handleError(error: AxiosError): Error {
    if (error.response) {
      ...
      const message = (error.response.data as { message?: string }).message || 'Une erreur est survenue';
      return new Error(message);
    } 
    if (error.request) {
      // La requête a été effectuée mais pas de réponse reçue
      return new Error('Le serveur ne répond pas. Veuillez réessayer plus tard.');
    }
